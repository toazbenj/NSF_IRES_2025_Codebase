...
PurePursuit::PurePursuit() : Node("pure_pursuit_node") {
    ...
    // this->declare_parameter("waypoints_path", "f1tenth_ws/src/pure_pursuit/racelines/e7_floor5.csv");
...
void PurePursuit::selected_waypoints_callback(const nav_msgs::msg::Path::SharedPtr path_msg){
    ...
    for (const auto& pose_stamped : path_msg->poses) {
        ...
        // double v = commanded_speed > 0.0 ? commanded_speed : 1.0; 
...
void PurePursuit::load_waypoints() {
    ...
    //testing
    // waypoints = csvFileData();  // Resets to default state
    // num_waypoints = 0;
    // average_dist_between_waypoints = 0;
}
